...
# ===  store__inGrid3D                                         === #
# ================================================================ #
def store__inGrid3D( Data=None, x1MinMaxNum=None, x2MinMaxNum=None, x3MinMaxNum=None, \
                     size=None, xyzRange=None ):
    # ------------------------------------------------- #
    # --- [1]   引数チェック                        --- #
    # ------------------------------------------------- #
    if ( Data        is None ): sys.exit( "[store__inGrid3D] Data ???" )
    if ( ( size is not None ) and ( xyzRange is not None ) ):
        ...
        x3MinMaxNum = np.array( [ xyzRange[0,2], xyzRange[1,2], size[0] ] )
    if ( x1MinMaxNum is None ): sys.exit( "[store__inGrid3D] x1MinMaxNum == ???" )
    if ( x2MinMaxNum is None ): sys.exit( "[store__inGrid3D] x2MinMaxNum == ???" )
    if ( x3MinMaxNum is None ): sys.exit( "[store__inGrid3D] x3MinMaxNum == ???" )
    ...
    #  -- [2-1] 使用する引数を準備                  --  #
    LI,LJ,LK  = int( x1MinMaxNum[2] ), int( x2MinMaxNum[2] ), int( x3MinMaxNum[2] )
    nData     = Data.shape[0]
    nCmp      = Data.shape[1]
    ...
    #  -- [3-2] 入出力管理                          --  #
    pyLIB.store__ingrid3d_.argtypes = [
    ...
    pyLIB.store__ingrid3d_.restype = ctypes.c_void_p

    # ------------------------------------------------- #
    # --- [4]   関数呼出 / 返却                     --- #
    # ------------------------------------------------- #
    pyLIB.store__ingrid3d_( Data_, ret_, nData_, nCmp_, LI_, LJ_, LK_, delta_, xyzRange_ )
    return( ret_ )
...
if ( __name__=='__main__' ):
    ...
    radii       = np.sqrt( ret[:,0]**2 + ret[:,1]**2 + ret[:,2]**2 )
    Data        = np.zeros( (ret.shape[0],5) )
    Data[:,0:3] = ret
    Data[:,  3] = np.exp( - 0.5 * radii**2 )
    Data[:,  4] = Data[:,0]
    Data        = Data[ np.where( radii < radius ) ]
    ret         = store__inGrid3D( Data=Data, x1MinMaxNum=x1MinMaxNum, \
